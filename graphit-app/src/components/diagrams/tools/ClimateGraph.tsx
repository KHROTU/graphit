...
const DEFAULT_CUSTOM_TEMP = '15, 16, 18, 22, 25, 28, 30, 29, 26, 21, 17, 14';
const DEFAULT_CUSTOM_PRECIP = '50, 45, 60, 80, 70, 40, 20, 20, 50, 80, 90, 60';
export default function ClimateGraph(props: ClimateGraphProps) {
  const initialState: State = { 
      preset: props.initialPreset || 'rainforest',
      customTempStr: props.initialCustomData?.temp || DEFAULT_CUSTOM_TEMP,
      customPrecipStr: props.initialCustomData?.precip || DEFAULT_CUSTOM_PRECIP,
  };
  ...
  const handleReset = () => {
    dispatch({ type: 'SET_PRESET', payload: 'rainforest' });
    dispatch({ type: 'SET_CUSTOM_TEMP', payload: DEFAULT_CUSTOM_TEMP });
    dispatch({ type: 'SET_CUSTOM_PRECIP', payload: DEFAULT_CUSTOM_PRECIP });
  };
  return (
    <DiagramErrorBoundary diagramName="Climate Graphs">
        ...
        <div className="md:col-span-1">
          <Card>
            ...
            <div className="p-6 space-y-6">
                ...
                <DiagramToolbar 
                  diagramName="Climate Graphs" 
                  getDiagramState={getDiagramState} 
                  onExport={() => openExportModal(diagramContainerRef, 'climate-graph')}
                  onReset={handleReset}
                />
            </div>
          </Card>
