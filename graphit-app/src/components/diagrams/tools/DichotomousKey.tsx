...
import { Plus, Trash2, GitBranch, Leaf } from 'lucide-react';
...
function keyReducer(state: State, action: Action): State {
    switch(action.type) {
        ...
        case 'RESET': {
            return { ...DEFAULT_STATE, nodes: { ...DEFAULT_STATE.nodes } };
        }
...
export default function DichotomousKey() {
  const [state, dispatch] = useReducer(keyReducer, DEFAULT_STATE);
  ...
  return (
    <DiagramErrorBoundary diagramName="Dichotomous Key">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1">
          <Card>
            ...
            <div className="p-6 border-t border-neutral-dark/30">
              <DiagramToolbar
                diagramName="Dichotomous Key"
                getDiagramState={getDiagramState}
                onExport={() => openExportModal(diagramContainerRef, 'dichotomous-key')}
                onReset={handleReset}
              />
            </div>
          </Card>
