...
export default function EnergyProfileDiagram(props: EnergyProfileProps) {
  ...
  const handleReset = () => {
    dispatch({ type: 'SET_ACTIVATION_ENERGY', payload: initialState.activationEnergy });
    dispatch({ type: 'SET_DELTA_H', payload: initialState.deltaH });
  };
  return (
    <DiagramErrorBoundary diagramName="Energy Profile Diagrams">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md-col-span-1">
          <Card>
            <CardHeader><CardTitle>Reaction Parameters</CardTitle></CardHeader>
            <div className="p-6 space-y-6">
                <SliderControl label="Activation Energy (Ea)" value={activationEnergy} min={10} max={100} step={1} unit="kJ/mol" onChange={(val) => dispatch({ type: 'SET_ACTIVATION_ENERGY', payload: val })} />
                <SliderControl label="Enthalpy Change (\u0394H)" value={deltaH} min={-50} max={50} step={1} unit="kJ/mol" onChange={(val) => dispatch({ type: 'SET_DELTA_H', payload: val })} />
                ...
                <DiagramToolbar 
                  diagramName="Energy Profile Diagrams" 
                  getDiagramState={getDiagramState} 
                  onExport={() => openExportModal(diagramContainerRef, 'energy-profile-diagram')}
                  onReset={handleReset}
                />
            </div>
          </Card>
          ...
          <Card className="h-full !p-4" ref={diagramContainerRef} data-testid="diagram-container">
              <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                      ...
                      <XAxis type="number" dataKey="progress" domain={[0, 100]} tick={false} axisLine={false} stroke="var(--color-text)" label={{ value: 'Reaction Progress', position: 'insideBottom', offset: -10, fill: 'var(--color-text)' }} />
