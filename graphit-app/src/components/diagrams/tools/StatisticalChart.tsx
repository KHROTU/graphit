...
const COLORS = ['var(--color-accent)', 'var(--color-secondary)', '#34d399', '#a78bfa', '#facc15', '#fb923c'];
...
export default function StatisticalChart() {
  ...
  const renderChart = () => {
    ...
    if (chartType === 'histogram') {
        return (
            <BarChart data={histogramData} barGap={0}>
                ...
                <XAxis dataKey="range" label={{ value: 'Value Range', position: 'insideBottom', offset: -5 }} stroke="var(--color-text)" tick={{ fill: 'var(--color-text)' }} />
                <YAxis allowDecimals={false} label={{ value: 'Frequency', angle: -90, position: 'insideLeft' }} stroke="var(--color-text)" tick={{ fill: 'var(--color-text)' }} />
                <Tooltip contentStyle={tooltipStyle} />
    ...
    return (
      <BarChart data={data}>
        ...
        <YAxis stroke="var(--color-text)" tick={{ fill: 'var(--color-text)' }} />
        <Tooltip contentStyle={tooltipStyle} />
        <Legend wrapperStyle={{ color: 'var(--color-text)' }} />
  ...
  return (
    <DiagramErrorBoundary diagramName="Statistical Charts">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-1">
          <Card>
            ...
            <DiagramToolbar
              diagramName="Statistical Charts"
              getDiagramState={getDiagramState}
              onExport={() => openExportModal(diagramContainerRef, 'statistical-chart')}
              onReset={handleReset}
            />
          </Card>
        </div>
