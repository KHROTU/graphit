...
export default function AdAsDiagram(props: AdAsProps) {
  ...
  const handleReset = () => {
    dispatch({ type: 'SET_AD_SHIFT', payload: 0 });
    dispatch({ type: 'SET_SRAS_SHIFT', payload: 0 });
    dispatch({ type: 'SET_LRAS_POS', payload: 80 });
    dispatch({ type: 'SET_MODEL', payload: 'classical' });
  };
  return (
    <DiagramErrorBoundary diagramName="AD/AS Diagram">
        ...
        <div className="md:col-span-1">
          <Card>
            ...
            <div className="p-6 space-y-6">
                ...
                />
                <SliderControl label="AD Shift" value={adShift} min={-50} max={50} step={1} onChange={(val) => dispatch({type: 'SET_AD_SHIFT', payload: val})} />
                {model === 'classical' ? (
                    <>
                      <SliderControl label="SRAS Shift" value={srasShift} min={-50} max={50} step={1} onChange={(val) => dispatch({type: 'SET_SRAS_SHIFT', payload: val})} />
                      <SliderControl label="LRAS Position (Full Employment)" value={lrasPosition} min={50} max={120} step={1} onChange={(val) => dispatch({type: 'SET_LRAS_POS', payload: val})} />
                    </>
                ) : (
                    <SliderControl label="Full Employment Level (Yfe)" value={lrasPosition} min={50} max={120} step={1} onChange={(val) => dispatch({type: 'SET_LRAS_POS', payload: val})} />
                )}
                <DiagramToolbar 
                  diagramName="AD/AS Diagram" 
                  getDiagramState={getDiagramState} 
                  onExport={() => openExportModal(diagramContainerRef, 'ad-as-diagram')}
                  onReset={handleReset}
                />
            </div>
          </Card>
