...
const tabOptions: { value: Tab; label: string }[] = [
  { value: 'mass', label: 'Mass Spec' },
  { value: 'ir', label: 'IR Spec' },
];
const massSpecData = {
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={chartData} margin={{ top: 20, right: 20, left: 10, bottom: 20 }}>
        ...
        <XAxis dataKey="mz" stroke="var(--color-text)" label={{ value: 'Mass/Charge (m/z)', position: 'insideBottom', offset: -10, fill: 'var(--color-text)' }}/>
        <YAxis stroke="var(--color-text)" label={{ value: 'Relative Abundance', angle: -90, position: 'insideLeft', fill: 'var(--color-text)' }} domain={[0, 110]}/>
        <Tooltip formatter={formatValue} cursor={{fill: 'transparent'}} contentStyle={{ backgroundColor: 'var(--color-neutral)', border: '1px solid var(--color-neutral-dark)', borderRadius: 'var(--border-radius-apple)' }} />
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <LineChart data={chartData} margin={{ top: 5, right: 20, left: 20, bottom: 20 }}>
        <CartesianGrid strokeDasharray="3 3" strokeOpacity={0.2} />
        <XAxis type="number" dataKey="wavenumber" domain={[4000, 500]} reversed={true} stroke="var(--color-text)" label={{ value: 'Wavenumber (cm\u207B\u00B9)', position: 'insideBottom', offset: -10, fill: 'var(--color-text)' }} />
        <YAxis domain={[0, 100]} reversed={true} stroke="var(--color-text)" label={{ value: 'Transmittance (%)', angle: -90, position: 'insideLeft', fill: 'var(--color-text)' }} />
...
export default function SpectroscopyTool(props: SpectroscopyProps) {
  ...
  const handleReset = () => {
    ...
    dispatch({ type: 'SET_MASS_MOLECULE', payload: initialState.massMolecule });
    dispatch({ type: 'SET_IR_MOLECULE', payload: initialState.irMolecule });
  };
  return (
    <DiagramErrorBoundary diagramName="Analytical Spectroscopy">
        ...
        <div className="md:col-span-1">
          <Card>
            ...
            <div className="p-4 border-t border-neutral-dark/30">
              <DiagramToolbar
                diagramName="Analytical Spectroscopy"
                getDiagramState={getDiagramState}
                onExport={() => openExportModal(diagramContainerRef, 'spectroscopy-tool')}
                onReset={handleReset}
              />
            </div>
          </Card>
