...
const sourceShapes: Record<SourceShape, React.FC> = { ambulance: AmbulanceShape, rectangle: RectangleShape };
...
export default function DopplerEffectSound(props: DopplerProps) {
  const initialState: State = {
    waveCount: props.initialWaveCount ?? defaults.waveCount,
    waveSpacing: props.initialWaveSpacing ?? defaults.waveSpacing,
    sourceVelocity: props.initialSourceVelocity ?? defaults.sourceVelocity,
    sourceShape: props.initialSourceShape ?? defaults.sourceShape,
    position: props.initialPosition ?? defaults.position,
    isPlaying: false,
  };
  ...
  const getDiagramState = () => ({
    initialWaveCount: state.waveCount,
    initialWaveSpacing: state.waveSpacing,
    initialSourceVelocity: state.sourceVelocity,
    initialSourceShape: state.sourceShape,
  ...
  return (
    <DiagramErrorBoundary diagramName="Doppler Effect Simulator">
          ...
          <Card>
            ...
            <div className="p-6 space-y-6">
              <div className="flex gap-2">
                <Button onClick={() => dispatch({type: 'TOGGLE_PLAY'})} variant="outline" className="w-full">
                  {isPlaying ? <Pause className="mr-2 h-4 w-4" /> : <Play className="mr-2 h-4 w-4" />}
                  {isPlaying ? 'Pause' : 'Play'}
                </Button>
                <Button onClick={() => dispatch({type: 'RESET'})} variant="outline" size="icon">
              ...
              <DiagramToolbar
                diagramName="Doppler Effect Simulator"
                getDiagramState={getDiagramState}
                onExport={() => openExportModal(diagramContainerRef, 'doppler-effect-sound')}
                onReset={handleReset}
              />
            </div>
          </Card>
