...
// Comprehensive data for Periods 1-6
const periodicData = {
  '1': {
    elements: ['H', 'He'],
    ionisation: [1312, 2372],
    radius: [53, 31],
    electronegativity: [2.20, null],
  },
  '2': {
    elements: ['Li', 'Be', 'B', 'C', 'N', 'O', 'F', 'Ne'],
    ionisation: [520, 900, 801, 1087, 1402, 1314, 1681, 2081],
    radius: [167, 112, 87, 67, 56, 48, 42, 38],
    electronegativity: [0.98, 1.57, 2.04, 2.55, 3.04, 3.44, 3.98, null],
  },
  '3': {
    elements: ['Na', 'Mg', 'Al', 'Si', 'P', 'S', 'Cl', 'Ar'],
    ionisation: [496, 738, 578, 789, 1012, 1000, 1251, 1521],
    radius: [190, 145, 118, 111, 98, 88, 79, 71],
    electronegativity: [0.93, 1.31, 1.61, 1.90, 2.19, 2.58, 3.16, null],
  },
  '4': {
    elements: ['K', 'Ca', 'Ga', 'Ge', 'As', 'Se', 'Br', 'Kr'],
    ionisation: [419, 590, 579, 762, 947, 941, 1140, 1351],
    radius: [243, 194, 136, 125, 114, 103, 94, 88],
    electronegativity: [0.82, 1.00, 1.81, 2.01, 2.18, 2.55, 2.96, 3.00],
  },
  '5': {
    elements: ['Rb', 'Sr', 'In', 'Sn', 'Sb', 'Te', 'I', 'Xe'],
    ionisation: [403, 550, 558, 709, 834, 869, 1008, 1170],
    radius: [265, 219, 156, 145, 133, 123, 115, 108],
    electronegativity: [0.82, 0.95, 1.78, 1.96, 2.05, 2.1, 2.66, 2.60],
  },
  '6': {
    elements: ['Cs', 'Ba', 'Tl', 'Pb', 'Bi', 'Po', 'At', 'Rn'],
    ionisation: [376, 503, 589, 716, 703, 812, 890, 1037],
    radius: [298, 253, 156, 154, 143, 135, 127, 120],
    electronegativity: [0.79, 0.89, 1.62, 2.33, 2.02, 2.0, 2.2, 2.2],
  },
};
...
export default function PeriodicTrendsGraph({ initialSelectedTrend = 'ionisation', initialSelectedPeriod = '3' }: PeriodicTrendsProps) {
  const [selectedTrend, setSelectedTrend] = useState<Trend>(initialSelectedTrend);
  const [selectedPeriod, setSelectedPeriod] = useState<Period>(initialSelectedPeriod);
          ...
          <div className="p-6 space-y-6">
            <div>
              ...
              <div className="flex flex-col gap-2 mt-2">
                <Button variant={selectedTrend === 'ionisation' ? 'default' : 'outline'} onClick={() => setSelectedTrend('ionisation')}>Ionisation Energy</Button>
                <Button variant={selectedTrend === 'radius' ? 'default' : 'outline'} onClick={() => setSelectedTrend('radius')}>Atomic Radius</Button>
                <Button variant={selectedTrend === 'electronegativity' ? 'default' : 'outline'} onClick={() => setSelectedTrend('electronegativity')}>Electronegativity</Button>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2 mt-2">
                {(Object.keys(periodicData) as Period[]).map(p => (
                  <Button key={p} variant={selectedPeriod === p ? 'default' : 'outline'} onClick={() => setSelectedPeriod(p)}>{`Period ${p}`}</Button>
                ))}
              </div>
            </div>
