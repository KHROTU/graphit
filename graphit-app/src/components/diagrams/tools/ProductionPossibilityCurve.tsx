...
export default function ProductionPossibilityCurve(props: PPCProps) {
  ...
  const handleReset = () => {
    dispatch({ type: 'SET_GROWTH', payload: 0 });
    dispatch({ type: 'SET_BOWEDNESS', payload: 2 });
    dispatch({ type: 'SET_POINT', payload: { x: 75, y: 75 } });
  };
  return (
    <DiagramErrorBoundary diagramName="Production Possibility Curves">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-1">
          <Card>
            <CardHeader><CardTitle>Configuration</CardTitle></CardHeader>
            <div className="p-6 space-y-6">
              <SliderControl label="Economic Growth (Shift)" value={growth} min={-100} max={50} step={1} onChange={(val) => dispatch({type: 'SET_GROWTH', payload: val})} />
              <SliderControl label="Opportunity Cost (Bowedness)" value={bowedness} min={1} max={5} step={0.1} onChange={(val) => dispatch({type: 'SET_BOWEDNESS', payload: val})} />
              ...
              <DiagramToolbar 
                diagramName="Production Possibility Curves" 
                getDiagramState={getDiagramState} 
                onExport={() => openExportModal(diagramContainerRef, 'production-possibility-curve')}
                onReset={handleReset}
              />
            </div>
          </Card>
