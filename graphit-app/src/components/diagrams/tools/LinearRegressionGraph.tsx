...
export default function LinearRegressionGraph({ initialCsvData = defaultData }: LinearRegressionProps) {
  ...
  const { data, headers, regressionLine, equation } = useMemo(() => {
    ...
    if (lines.length < 2) return { data: [] as { x: number; y: number }[], headers: headerRow, regressionLine: null as { x: number; y: number }[] | null, equation: '' };
  ...
  return (
    <DiagramErrorBoundary diagramName="Scatter Plot & Regression">
        ...
        <div className="md:col-span-1">
          <Card>
            ...
            <div className="p-6 space-y-4">
              ...
              <DiagramToolbar
                diagramName="Scatter Diagrams & Regression"
                getDiagramState={getDiagramState}
                onExport={() => openExportModal(diagramContainerRef, 'linear-regression-graph')}
                onReset={handleReset}
              />
            </div>
          </Card>
          ...
          <Card className="h-full !p-4" ref={diagramContainerRef} data-testid="diagram-container">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                  ...
                  <XAxis type="number" dataKey="x" name={headers[0]} domain={['dataMin', 'dataMax']} label={{ value: headers[0], position: 'insideBottom', offset: -10 }} stroke="var(--color-text)" tick={{ fill: 'var(--color-text)' }} />
                  <YAxis type="number" dataKey="y" name={headers[1]} domain={['dataMin', 'dataMax']} label={{ value: headers[1], angle: -90, position: 'insideLeft' }} stroke="var(--color-text)" tick={{ fill: 'var(--color-text)' }} />
