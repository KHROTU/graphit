...
const defaultColors = ['var(--color-accent)', 'var(--color-secondary)', '#34d399', '#a78bfa', '#facc15'];
...
export default function FunctionGraph() {
  ...
  const data = useMemo(() => {
    ...
    for (let x = xDomain.min; x <= xDomain.max; x += step) {
      const point: { [key: string]: number } = { x: Number(x.toFixed(4)) };
      plots.forEach(plot => {
        try {
          point[plot.id] = parse(plot.equation).compile().evaluate({ x }) as number;
  ...
  const getDiagramState = () => ({
    plots: plots.map(p => ({ equation: p.equation, color: p.color })),
    xDomain: { ...xDomain },
    yDomain: { ...yDomain },
  });
  const handleReset = () => {
  ...
  return (
    <DiagramErrorBoundary diagramName="Function Graph Plotter">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-1">
          <Card>
            ...
            <DiagramToolbar
              diagramName="Function Graph Plotter"
              getDiagramState={getDiagramState}
              onExport={() => openExportModal(diagramContainerRef, 'function-graph')}
              onReset={handleReset}
            />
          </Card>
        </div>
          ...
          <Card className="h-full !p-4" ref={diagramContainerRef} data-testid="diagram-container">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={data} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                  ...
                  <XAxis type="number" dataKey="x" domain={[xDomain.min, xDomain.max]} allowDataOverflow stroke="var(--color-text)" tick={{ fill: 'var(--color-text)' }} />
                  <YAxis type="number" domain={[yDomain.min, yDomain.max]} allowDataOverflow stroke="var(--color-text)" tick={{ fill: 'var(--color-text)' }} />
                  <Tooltip formatter={formatValue} labelFormatter={v => `x = ${formatValue(v)}`} contentStyle={tooltipStyle} />
