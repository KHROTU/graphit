...
export default function CircuitDiagram() {
    const [state, dispatch] = useReducer(circuitReducer, defaults);
  ...
  return (
    <DiagramErrorBoundary diagramName="Circuit Diagram">
          ...
          <Card>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="text-sm border-t border-neutral-dark/50 pt-4">
                ...
                <p>Total Resistance: <span className="font-mono text-accent">{formatValue(totalResistance)} Ω</span></p>
                <p>Total Current: <span className="font-mono text-secondary">{formatValue(current)} A</span></p>
              </div>
            </div>
          </Card>
          <Card>
            ...
            <div className="p-4 space-y-4 max-h-[400px] overflow-y-auto">
              <Label>Series</Label>
              {seriesResistors.map(r => (
                <ResistorInput key={r.id} value={r.value} onChange={(val) => dispatch({type: 'UPDATE_RESISTOR', payload: {id: r.id, value: val, type: 'series'}})} onRemove={() => dispatch({type: 'REMOVE_RESISTOR', payload: {id: r.id, type: 'series'}})} />
              ))}
              ...
              {parallelResistors.map(r => (
                <ResistorInput key={r.id} value={r.value} onChange={(val) => dispatch({type: 'UPDATE_RESISTOR', payload: {id: r.id, value: val, type: 'parallel'}})} onRemove={() => dispatch({type: 'REMOVE_RESISTOR', payload: {id: r.id, type: 'parallel'}})} />
              ))}
            ...
            <DiagramToolbar
              diagramName="Circuit Diagram"
              getDiagramState={getDiagramState}
              onExport={() => openExportModal(diagramContainerRef, 'circuit-diagram')}
              onReset={handleReset}
            />
          </Card>
        </div>
            ...
            <div ref={diagramContainerRef} data-testid="diagram-container" className="w-full h-full bg-neutral-dark/30 rounded-apple">
              <svg viewBox="0 0 400 300" className="w-full h-full text-text">
                ...
                {parallelResistors.map((r, i) => {
                  ...
                  return (
                    <g key={r.id}>
                      <path d={`M 200 110 V ${yPos} H 250 M 300 ${yPos} H 350 V 110`} stroke="currentColor" strokeWidth="2" fill="none" />
                      <path d={`M 200 190 V ${yPos} H 250 M 300 ${yPos} H 350 V 190`} stroke="currentColor" strokeWidth="2" fill="none" />
                      <ResistorShape x={275} y={yPos} label={`${r.value}Ω`} />
                    </g>
                  );
                })}
              </svg>
