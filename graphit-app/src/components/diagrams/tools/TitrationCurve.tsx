...
export default function TitrationCurve(props: TitrationProps) {
  ...
  const handleReset = () => {
    dispatch({ type: 'SET_ANALYTE_VOLUME', payload: initialState.analyteVolume });
    dispatch({ type: 'SET_ANALYTE_CONC', payload: initialState.analyteConc });
    dispatch({ type: 'SET_TITRANT_CONC', payload: initialState.titrantConc });
  };
  return (
    <DiagramErrorBoundary diagramName="Titration Curve Plotter">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-1">
          <Card>
            <CardHeader><CardTitle>Titration Parameters</CardTitle></CardHeader>
            <div className="p-6 space-y-6">
                <SliderControl label="Analyte (Acid) Volume (mL)" value={analyteVolume} min={10} max={50} step={1} onChange={(val) => dispatch({type: 'SET_ANALYTE_VOLUME', payload: val})} />
                <SliderControl label="Analyte (Acid) Conc. (M)" value={analyteConc} min={0.05} max={0.5} step={0.01} onChange={(val) => dispatch({type: 'SET_ANALYTE_CONC', payload: val})} />
                <SliderControl label="Titrant (Base) Conc. (M)" value={titrantConc} min={0.05} max={0.5} step={0.01} onChange={(val) => dispatch({type: 'SET_TITRANT_CONC', payload: val})} />
                ...
                <DiagramToolbar 
                  diagramName="Titration Curve Plotter" 
                  getDiagramState={getDiagramState} 
                  onExport={() => openExportModal(diagramContainerRef, 'titration-curve')}
                  onReset={handleReset}
                />
            </div>
          </Card>
